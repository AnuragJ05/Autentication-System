...
class recognizeuser():
 ...
 def recog(self):
    ...
    time.sleep(2.0)
    self.sendResponse("Speak your name \n recording...")
    frames = []

    for i in range(0, int(RATE / CHUNK * RECORD_SECONDS)):
        data = stream.read(CHUNK)
        frames.append(data)
    self.sendResponse("finished recording")
    ...
    if len(models) == 0:
        self.sendResponse("No Users in the Database!")
        return
    ...
    if identity == 'unknown':
            self.sendResponse("Not Recognized! Try again...")
            return
    
    self.sendResponse(f"Recognized as -  {identity}")

    # face recognition
    self.sendResponse("Keep Your face infront of the camera")
    ...
    while True:
        ...
        if len(face) == 1:
            ...
            if min_dist <= 0.4 and name == identity:
                self.sendResponse("Door Unlocked! Welcome " + str(name))
                break
